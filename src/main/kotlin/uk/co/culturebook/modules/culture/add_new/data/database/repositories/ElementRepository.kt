...
object ElementRepository : ElementDao {
    ...
    private fun resultSetToElements(resultSet: ResultSet) = resultSet.use { resultSet ->
        ...
        while (resultSet.next()) {
            ...
            elements += Element(
                id = resultSet.getString(Elements.id.name).generateUUID(),
    ...
    override suspend fun getDuplicateElement(name: String, type: ElementType): List<Element> = dbQuery {
        val query = if (currentDialect is PostgreSQLDialect) {
            ...
            WHERE ${Elements.name.name} % '$name'
            ORDER BY distance DESC""".trimIndent()
        } else {
            """
            SELECT *, MY_SIMILARITY(${Elements.name.name}, '$name') as distance
            FROM ${Elements.tableName}
            WHERE MY_SIMILARITY(${Elements.name.name}, '$name') > 0.8
            ORDER BY distance DESC""".trimIndent()
        }
